...
document
  ...
  .addEventListener("change", atualizarSelects);
document
  .getElementById("selectUsuario")
  .addEventListener("change", atualizarSelects);
...
async function atualizarSelects() {
  ...
  const clientesFiltrados = clientes.filter(
    (c) => c.nome !== nomeUsuarioSelecionado
  );
  const usuariosFiltrados = usuarios.filter(
    (u) => u.nome !== nomeClienteSelecionado
  );
  ...
  selectCliente.value =
    clientesFiltrados.find((c) => c.nome === nomeClienteSelecionado)?.id || "";
  selectUsuario.value =
    usuariosFiltrados.find((u) => u.nome === nomeUsuarioSelecionado)
      ?.idUsuario || "";
}
...
async function adicionarProduto() {
  ...
  if (itemExistente) {
    ...
    const qtdDisponivel = await verificaQuantidade(
      dto.idProduto,
...
async function atualizarTabela(itens) {
  try {
    ...
    itens.forEach((item) => {
      if (item.quantidade > 0) {
        ...
        tr.innerHTML = `
                ...
                <td>R$ ${preco.toLocaleString("pt-BR", {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2,
                })}</td>
                <td>R$ ${somaItem.toLocaleString("pt-BR", {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2,
                })}</td>
            `;
    ...
    trTotal.innerHTML = `
    <td colspan="3"><strong>Total</strong></td>
    <td><strong>R$ ${subtotal.toLocaleString("pt-BR", {
      minimumFractionDigits: 2,
      maximumFractionDigits: 2,
    })}</td></strong></td>
`;
...
async function fecharVenda() {
  if (!vendaAtual || itens.length === 0) {
    alert(
      "Não foi possível fechar a venda: Não há venda em andamento ou produtos adicionados."
    );
    return;
  }
  ...
  try {
    const resposta = await fetch(
      `http://localhost:5164/BlueMoon/Vendas/${vendaAtual.id}/Itens`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
async function cancelarVenda() {
  ...
  try {
    const resposta = await fetch(
      `http://localhost:5164/BlueMoon/Vendas/${vendaAtual.id}/Cancelar`,
      {
        method: "PATCH",
      }
    );
