...
async function carregarVenda() {
  ...
  vendas.forEach((v) => {
    ...
    if (
      (v.dataFaturamento === "01/01/0001 00:00:00" &&
        situacao[v.situacao] === "FECHADA") ||
      situacao[v.situacao] === "ABERTA"
    ) {
      v.dataFaturamento = "Não Faturada!";
    } else if (
      v.dataFaturamento === "01/01/0001 00:00:00" &&
      situacao[v.situacao] === "CANCELADA"
    ) {
      v.dataFaturamento = "Venda Cancelada!";
    } else if (
      v.dataFaturamento === "01/01/0001 00:00:00" &&
      situacao[v.situacao] === "ESTORNADA"
    ) {
    ...
    if (situacao[v.situacao] === "ABERTA") {
      ...
      <td style="text-align: center">${
        situacao[v.situacao] || "INDEFINIDO"
      }</td>
      <td class="text-center">
          <button class="btn btn-primary" data-toggle="tooltip" data-placement="top" title="Visualizar Venda" onclick="visualizarVenda('${
            v.id
          }')"><img src="../../assets/View.png"></button>
          <button class="btn btn-warning" data-toggle="tooltip" data-placement="top" title="Retomar Venda" onclick="retomarVenda('${
            v.id
    ...
    `;
      ...
      <td class="text-center">
          ...
          <button class="btn btn-success" data-toggle="tooltip" data-placement="top" title="Finalizar Venda" onclick="finalizarVenda('${
            v.id
          }')"><img src="../../assets/finalizarVenda.png"></button>
      </td>
    `;
      tbody.appendChild(linha);
      ...
      <td class="text-center">
          ...
          <button class="btn btn-warning" data-toggle="tooltip" data-placement="top" title="Estornar Venda" onclick="estornarVenda('${
            v.id
          }')"><img src="../../assets/EstornoIcon.png"></button>
      </td>
    `;
      tbody.appendChild(linha);
    } else {
...
document
  ...
  .addEventListener("submit", async function (event) {
    ...
    const dataConvertida = dataAbertura
      ? converterDataParaBack(dataAbertura)
      : "";
    ...
    if (!codigo) {
      dto = {
        ...
        situacao:
          situacao !== "" && !isNaN(parseInt(situacao))
            ? parseInt(situacao)
            : "",
      };
    } else {
    ...
    try {
      const resposta = await fetch(
        "http://localhost:5164/BlueMoon/Vendas/Search",
        {
          method: "POST",
      ...
      vendas.forEach((v) => {
        ...
        if (
          (v.dataFaturamento === "01/01/0001 00:00:00" &&
            situacaoDescricao[v.situacao] === "FECHADA") ||
          situacao[v.situacao] === "ABERTA"
        ) {
          v.dataFaturamento = "Não Faturada!";
        } else if (
          v.dataFaturamento === "01/01/0001 00:00:00" &&
          situacaoDescricao[v.situacao] === "CANCELADA"
        ) {
          v.dataFaturamento = "Venda Cancelada!";
        } else if (
          v.dataFaturamento === "01/01/0001 00:00:00" &&
          situacaoDescricao[v.situacao] === "ESTORNADA"
        ) {
          v.dataFaturamento = "Venda Estornada!";
        }
      ...
      <td style="text-align: center">${
        situacaoDescricao[v.situacao] || "INDEFINIDO"
      }</td>
      <td class="text-center">
          <button class="btn btn-primary" data-toggle="tooltip" data-placement="top" title="Visualizar Venda" onclick="visualizarVenda('${
            v.id
...
async function estornarVenda(idVenda) {
  try {
    ...
    const resposta = await fetch(
      `http://localhost:5164/BlueMoon/Vendas/${idVenda}/Estornar`,
      {
        method: "PATCH",
      }
    );
