...
function preencherCampos(venda) {
  ...
  if (
    (venda.dataFaturamento === "01/01/0001 00:00:00" &&
      situacao[venda.situacao] === "FECHADA") ||
    situacao[venda.situacao] === "ABERTA"
  ) {
    venda.dataFaturamento = "Não Faturada!";
  } else if (
    venda.dataFaturamento === "01/01/0001 00:00:00" &&
    situacao[venda.situacao] === "CANCELADA"
  ) {
    venda.dataFaturamento = "Venda Cancelada!";
  } else if (
    venda.dataFaturamento === "01/01/0001 00:00:00" &&
    situacao[venda.situacao] === "ESTORNADA"
  ) {
  ...
  document.getElementById("inputDataFaturamento").value =
    venda.dataFaturamento || "";

  document.getElementById("inputSituacao").value =
    situacao[venda.situacao] || "INDEFINIDO";
...
function preencherTabela(itens) {
  ...
  itens.forEach((item) => {
    ...
    tr.innerHTML = `
            ...
            <td>R$ ${item.produtoValorVenda.toLocaleString("pt-BR", {
