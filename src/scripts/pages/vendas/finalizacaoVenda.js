...
window.addEventListener("load", async () => {
  ...
  document
    .getElementById("btnFaturar")
    .addEventListener("click", () => faturarVenda(idVenda));
  document
    .getElementById("btnCancelar")
    .addEventListener("click", () => cancelarVenda(idVenda));
...
async function faturarVenda(idVenda) {
  try {
    const resposta = await fetch(
      `http://localhost:5164/BlueMoon/Vendas/${idVenda}/Faturar`,
      {
        method: "PATCH",
      }
    );
...
async function cancelarVenda(idVenda) {
  ...
  try {
    const resposta = await fetch(
      `http://localhost:5164/BlueMoon/Vendas/${idVenda}/Cancelar`,
      {
        method: "PATCH",
      }
    );
