...
async function carregarVenda(idVenda) {
  const resposta = await fetch(
    `http://localhost:5164/BlueMoon/Vendas/${idVenda}`
  );
  vendaAtual = await resposta.json();

  document.getElementById(
    "selectCliente"
  ).innerHTML = `<option selected>${vendaAtual.nomeCliente}</option>`;
  document.getElementById(
    "selectUsuario"
  ).innerHTML = `<option selected>${vendaAtual.nomeVendedor}</option>`;
...
async function adicionarProduto() {
  ...
  const itemExistente = itens.find(
    (item) => item.idProduto == produtoSelecionado.idProduto
  );
...
async function atualizarTabela(itensVenda) {
  ...
  itensVenda.forEach((item) => {
    ...
    tr.innerHTML = `
            ...
            <td>R$ ${soma.toLocaleString("pt-BR", {
              minimumFractionDigits: 2,
            })}</td>
        `;
